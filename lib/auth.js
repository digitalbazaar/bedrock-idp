...
bedrock.events.on('bedrock.init', function(callback) {
  if(bedrock.config.idp.keys.length > 0) {
    logger.warning(
      'Move bedrock.config.idp.keys values to bedrock.config.key.keys');
    // append old config keys to new config keys
    var keys = bedrock.config.key.keys;
    keys.push.apply(keys, bedrock.config.idp.keys);
  }
      ...
      request.post(bedrock.config.idp.owner.didRegistrationUrl, {
        ...
        if(err) {
          return callback(new BedrockError(
            'Error registering DID after proof of patience.',
            'RegisterDidError', {
              didRegistrationUrl: bedrock.config.idp.owner.didRegistrationUrl,
...
function _establishProofOfPatience(didDocument, callback) {
  request.post(bedrock.config.idp.owner.didRegistrationUrl, {
    ...
    if(err) {
      return callback(new BedrockError(
        'Error establishing proof of patience.', 'RegisterDidError', {
          didRegistrationUrl: bedrock.config.idp.owner.didRegistrationUrl,
