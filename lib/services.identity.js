...
function addRoutes(app) {
  ...
  app.post('/register/:token', parseForm, function(req, res, next) {
    var token = req.params.token;
    var identityCookie = JSON.parse(req.cookies[token]);
    var jsonPostData = JSON.parse(req.body.jsonPostData);
    var did = jsonPostData.id;
    var identityId = auth.createIdentityId(identityCookie.sysSlug);
    var identity = {
    ...
    async.waterfall([
      ...
      function(results, callback) {
        ...
        auth.login(req, res, next, function(err) {
          ...
          res.redirect(results.identity.id + '/dashboard');
        });
      }
    ], function(err) {
      return next(err);
