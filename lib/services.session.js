...
function addRoutes(app) {
      ...
      function(req, res, next) {
        async.waterfall([
          ...
          function(results, callback) {
            ...
            brPassport.login(req, res, next, function(err) {
              if(err) {
  ...
  app.post('/session/login',
    ...
    function(req, res, next) {
      brPassport.login(req, res, next, function(err, user, choice) {
        if(err) {
          return next(err);
        }
        var out = {};
  ...
  app.post('/session/password/reset',
    ...
    function(req, res, next) {
      ...
      async.waterfall([
        function(callback) {
          brAuth.resolveIdentityIdentifier(identifier, callback);
        },
        function(identityIds, callback) {
          ...
          async.until(function() {return success !== 0;}, function(callback) {
            ...
            identity.id = next;
            brAuth.setIdentityPassword({id: next}, identity, function(err) {
              if(!err) {
                success = 1;
              }
              callback();
...
api._createIdentity = function(options, req, callback) {
  ...
  async.auto({
    createIdentity: function(callback) {
      ...
      brIdentity.createIdentity(null, identity, function(err, record) {
        if(err) {
          return callback(err);
        }
